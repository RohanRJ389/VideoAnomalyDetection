...
def process_video_and_save_to_h5py(
    video_path, output_h5py_path, success_file_path, model_path="yolov5n.pt"
):
        ...
        with h5py.File(output_h5py_path, "w") as h5file:
            ...
            while cap.isOpened():
                ...
                try:
                    ...
                    print(
                        f"Error in tracking on frame {frame_count} in {video_path}: {e}"
                    )
                ...
                if results and results[0].boxes.id is not None:
                    ...
                    for i, (box, track_id, confidence, class_id) in enumerate(
                        zip(boxes, track_ids, confidences, class_ids)
                    ):
                        track_data_group = dataset.create_group(
                            f"frame_{frame_count:05}/track_{track_id}"
                        )
